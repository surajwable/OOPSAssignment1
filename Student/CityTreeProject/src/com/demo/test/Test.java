...
public class Test {
	...
	public static void displayMap(Map <String,List<String>> cmap) {
		if(cmap!=null) {
			//a set view of the keys contained in this map=keySet()
			//Keyset fetches only keys in the map
			for(String s: cmap.keySet()) {
				System.out.println("City: "+s);
				//cmap.get(s) gives values from map if key if passed to it
				//lst is a reference pointing to list of values.i.e trees
				List<String> lst = cmap.get(s);
				//to print the list of trees.
				for(int i=0;i<lst.size();i++) {
					//print like 1. sunflower
					//(i+1)is the index number which will increament 
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2:
				...
				//in lst there will be tree names
				//giving key cname to find values in the form of List of Strings
